...
from utils.forms import build_form
from utils.exceptions import RedirectException
from utils.decorators import render_to, ajax_request
from utils.helpers import get_object_or_404_ajax
from utils.http import render_response
...
def process_root_request(request):
    if settings.BLOG_URLCONF_ROOT:
        return HttpResponseRedirect('/%s' % settings.BLOG_URLCONF_ROOT)
    else:
        return post_list(request, {'paginate_by': settings.PAGINATE_BY})
